	...
	public function index() {
		...
		if ($this->input->post('is_submitted_continue')) { 
			...
			if ( empty($this->errors->errors) ){
				...
				if( '' == $post['feed_type'] and ! preg_match('%\W(xml|csv|zip|gz)$%i', trim($post['url']))) $post['feed_type'] = pmxi_get_remote_file_name(trim($post['url']));
				
				if ('zip' == $post['feed_type'] or '' == $post['feed_type'] and preg_match('%\W(zip)$%i', trim($post['url']))) {							
					...
					if (!file_exists($tmpname)) {										
						$request = get_file_curl($post['url'], $tmpname);
				...
				} elseif ('csv' == $post['feed_type'] or '' == $post['feed_type'] and preg_match('%\W(csv|txt|dat|psv)$%i', trim($post['url']))) {
					...
					$fileInfo = ('gz' == $post['feed_type'] or '' == $post['feed_type'] and preg_match('%\W(gz)$%i', trim($post['url']))) ? pmxi_gzfile_get_contents($post['url']) : pmxi_copy_url_file($post['url'], true);
			...
			if (empty($post['ftp']['url'])) {
				...
				if ( $is_ftp_ok ){							
					...
					$file_path_array = (strpos($filePath, "*") !== false) ? PMXI_Helper::safe_glob($filePath, PMXI_Helper::GLOB_NODIR | PMXI_Helper::GLOB_PATH) : array($filePath);					
					$local_paths = array();
					$csv_paths = array();
			...
			if (empty($post['file'])) {
			...
			} elseif (preg_match('%\W(csv|txt|dat|psv)$%i', trim($post['file']))) {
				$uploads = PMXI_Plugin::ROOT_DIR . '/upload/';
				...
				if ( preg_match('%\W(csv|txt|dat|psv)$%i', trim($path)) or (!empty($fileInfo) and $fileInfo['type'] == 'csv') ){																																					
					include_once(PMXI_Plugin::ROOT_DIR.'/libraries/XmlImportCsvParse.php');		
					$csv = new PMXI_CsvParser($filePath, true); // create chunks
					@unlink($filePath);
					$filePath = $csv->xml_path;
